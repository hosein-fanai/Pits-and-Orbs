...
class NormalizeObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.observation_space = spaces.Dict({
            ...
            **{f"player{i}_movements": spaces.Box(low=0., high=1., shape=(1,), dtype=np.float32) for i in range(players_num)},
            ...
            **{f"player{i}_has_orb": spaces.Box(low=0., high=1., shape=(1,), dtype=np.float32) for i in range(players_num)} | 
            ...
            ({"player_turn": spaces.Box(low=0., high=1., shape=(1,), dtype=np.float32)} if players_num > 1 else {})
        })
    ...
    def observation(self, observation):
        ...
        return OrderedDict([
            ...
            *((f"player{i}_movements", np.array([observation[f"player{i}_movements"]]) / self.env.max_movements) for i in range(players_num)),
