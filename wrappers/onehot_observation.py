...
class OnehotObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.observation_space = spaces.Dict({
            ...
            **{f"player{i}_movements": spaces.Box(low=0., high=1., shape=(self.env.max_movements,), dtype=np.float32) for i in range(players_num)}, 
    ...
    def observation(self, observation):
        ...
        return OrderedDict([
            ...
            *((f"player{i}_movements", self._onehot(np.array(observation[f"player{i}_movements"]), depth=self.env.max_movements)) for i in range(players_num)), 
