...
class OnehotObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.observation_space = spaces.Dict({
            ...
            **{f"player{i}_movements": spaces.Box(low=0., high=1., shape=(self.env.max_movements+1,), dtype=np.float32) for i in range(players_num)}, 
            ...
            **{f"player{i}_has_orb": spaces.Box(low=0, high=1, shape=(1,), dtype=np.uint8) for i in range(players_num)} | 
            ...
            ({"player_turn": spaces.Box(low=0, high=1, shape=(1,), dtype=np.uint8)} if players_num > 1 else {})
        })
    ...
    def observation(self, observation):
        ...
        return OrderedDict([
            ...
            *((f"player{i}_movements", self._onehot(np.array(observation[f"player{i}_movements"]), depth=self.env.max_movements+1)) for i in range(players_num)), 
            ...
            *((f"player{i}_has_orb", np.array([observation[f"player{i}_has_orb"]])) for i in range(players_num))] + 
            ...
            ([("player_turn", np.array([observation["player_turn"]]))] if players_num > 1 else [])
        )
